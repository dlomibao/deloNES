...
public class MapperCNROM implements Mapper {
    ...
    public int cpuMapWrite(int address) {
        // Address-only form: legacy callers see a no-PRG-RAM cart.
        return UNMAPPED;
    }

    @Override
    public int cpuMapWrite(int address, int value) {
        // Value-aware form: latches the CHR bank from any $8000-$FFFF
        // write, then returns UNMAPPED so the byte doesn't also land in
        // PRG-RAM (CNROM has no PRG-RAM). Bus-conflict handling is
        // intentionally omitted — see class javadoc.
        if (address >= 0x8000 && address <= 0xFFFF) {
