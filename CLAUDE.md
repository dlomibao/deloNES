...
### APU + audio output
- `components.APU` implements the audio processing unit: registers $4000-$4017, frame counter (4/5-step, IRQ), two pulse channels (envelope/sweep/length), triangle (linear counter), noise (LFSR), and DMC (sample fetch with cycle-exact CPU stalls, IRQ). Channel units live in `components.apu.*`. `ApuMixer` applies the NESdev nonlinear lookup-table mix, a fractional box-average downsampler to the host rate, and 90 Hz HP / 14 kHz LP filters into `ApuSampleBuffer` (float ring). Validated against the blargg apu_test/apu_reset/dmc_tests ROMs (committed under `core/src/test/resources/test-roms/blargg/`, see CREDITS.md).
- Host output: desktop `desktop.audio.AudioOut` (LibGDX AudioDevice, drained after each `runFrame()`); web `client.WebAudioOut` (TeaVM ScriptProcessorNode; `new AudioContext()` via @JSClass — the `.create()` binding is broken; capture-phase gesture resume for autoplay). Design decisions D1-D19 in `docs/apu-plan.md`.
...
### Rendering + launchers
- `render.PixelRenderer` draws an arbitrary RGBA pixel array via LibGDX. **Channel order (corrected 2026-07-18 — the old note here stated the inverse):** the PPU framebuffer ints (`getVisibleScreenPixels1D()` etc.) are **ARGB** (`0xAARRGGBB` — see `NES_COLOR_PALETTE`, `clearScreen()`); hosts convert to RGBA8888 with `(argb << 8) | (argb >>> 24)` before LibGDX upload. Getting this wrong flips alpha and causes invisible/garbled output (the 2026-01-01 bug class). The harness `ScreenCapture` consumes the ARGB ints natively.
